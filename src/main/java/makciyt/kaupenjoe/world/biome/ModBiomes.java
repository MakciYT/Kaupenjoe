package makciyt.kaupenjoe.world.biome;
...
public class ModBiomes {
    public static final DeferredRegister<Biome> BIOMES = DeferredRegister.create(ForgeRegistries.BIOMES, Kaupenjoe.MOD_ID);
    ...
    //Все это взято и брать отсюда: BiomeMaker.java
    //depth - высота где будет размещен биом
    //scale - высота самого биома (маленькое число=равнина, большое число=биом похожий на расширенный мир)
    //некоторые штуки тут нерабочие, хз почему, разберись...
    private static Biome makeRiftBiome(final Supplier<ConfiguredSurfaceBuilder<?>> surfaceBuilder, float depth, float scale) {
        ...
        return (new Biome.Builder()).precipitation(Biome.RainType.NONE).category(Biome.Category.DESERT).depth(depth).scale(scale)
                .temperature(1.5F).downfall(0.9F).setEffects((new BiomeAmbience.Builder()).setWaterColor(-3407872).setWaterFogColor(-16777216)
                        ...
                        .setMoodSound(new MoodSoundAmbience(SoundEvents.AMBIENT_WARPED_FOREST_MOOD, 6000, 8, 2.0D)) //- я ниче не слышу
