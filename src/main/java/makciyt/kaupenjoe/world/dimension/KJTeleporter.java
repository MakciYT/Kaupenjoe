...
import makciyt.kaupenjoe.block.custom.SpeakerBlock;
    ...
    public Entity placeEntity(Entity entity, ServerWorld currentWorld, ServerWorld destinationWorld,
        ...
        if (insideDimension) {
            ...
            for (BlockPos checkPos : BlockPos.getAllInBoxMutable(destinationPos.down(10).west(10), destinationPos.up(10).east(10))) {
                if (destinationWorld.getBlockState(checkPos).getBlock() instanceof SpeakerBlock) {
                    doSetBlock = false;
                    break;
                }
            }
            if (doSetBlock) {
                destinationWorld.setBlockState(destinationPos, ModBlocks.SPEAKER.get().getDefaultState());
            }
        }
